...
const Seekbar = () => {
  const {seekSliderRef, seekProgressRef, progress, seekMouseDown, seekChange, seekMouseEnter, seekMouseLeave } = useContext(PlayerContext)
  ...
  return (
    <div className='w-[524px] h-3 relative'>
      ...
      <div className='w-[512px] h-1 absolute left-[6px] top-1 rounded-full bg-[#4c4c4c] overflow-hidden'>
        <div ref={seekProgressRef} className='w-[512px] h-1 rounded-full bg-white' style={{transform: `translateX(calc(-100% + ${progress*100}%))`}}></div>
      </div>
    </div>
  )
}
