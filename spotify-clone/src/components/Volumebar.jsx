...
const Volumebar = () => {
  const {volumeSliderRef, volumeBarRef, volume, volumeMouseEnter, volumeMouseLeave, volumeChange } = useContext(PlayerContext)
  ...
  return (
    <div className='w-[105px] h-3 relative'>
      ...
      <div className='w-[93px] h-1 absolute left-[6px] top-1 rounded-full bg-[#4c4c4c] overflow-hidden'>
        <div ref={volumeBarRef} className='w-[93px] h-1 rounded-full bg-white' style={{transform: `translateX(calc(-101% + ${volume*100}%))`}}></div>
      </div>
    </div>
  )
}
